...
const imageHasLoaded = (img) =>
    !('naturalHeight' in img && img.naturalHeight + img.naturalWidth === 0) ||
 ...
 * @param emitOnLoad {Boolean} - Should promise fire image load event
 */
const imageLoaderPromise = (ctx, images, emitOnLoad = false) =>
    Promise.all(getImagePromises(ctx, images, emitOnLoad)).then(() => {
...
const setupEventListeners = (ctx) => {
    ...
    ctx.on(ctx.constants.EVENT_IMAGE_COMPLETE, () =>
        ctx.recalculate(true, true)
    );
 ...
 */
function getOptionsAsMobileFirst({
    options,
    responsiveOptions,
    keys,
    docWidth
...
function getResponsiveOptions(options) {
    let docWidth = options.useContainerForBreakpoints
...
function sort(ctx, $eles, refresh = false, markasComplete = true) {
    ...
    foreach($eles, (ele) => {
        ...
        ele.style[ctx.options.rtl ? 'right' : 'left'] =
            `${ctx.cols[ctx.lastcol]}`;
